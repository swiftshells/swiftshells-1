    ...
    <div className="space-y-8 pb-24">
      ...
      <div className="relative h-[160px] rounded-xl overflow-hidden shadow-md">
        ...
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
          {banners.map((_, idx) => (
            <div
              key={idx}
              className={`h-1 rounded-full transition-all ${idx === currentBanner ? "w-6 bg-white" : "w-2 bg-white/40"}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 border-y border-gray-100 dark:border-slate-800 py-2 flex items-center gap-3 px-4 shadow-sm">
        ...
        <div className="flex-1 overflow-hidden">
          <div className="animate-marquee whitespace-nowrap text-[13px] font-medium text-gray-600 dark:text-gray-300">
            🔥 100% Bonus Topup Event is Live! • Instant Delivery for all BD
            Server Top-ups • Trusted by 50k+ Gamers.
          </div>
        </div>
      </div>
      ...
      <section>
        <div className="flex items-center justify-center mb-6">
          <h2 className="text-xl font-extrabold text-[#2c3e50] dark:text-white tracking-tight uppercase">
            HOT PRODUCT
          </h2>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] opacity-80">
              Get best experience on mobile
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          <button className="bg-white text-[#ff8c00] px-4 py-1.5 rounded-lg text-xs font-bold shadow-sm">
            Install
          </button>
