...
import { Mail, Lock, User as UserIcon, Phone, Loader2, ArrowLeft, AlertCircle, Inbox } from 'lucide-react';
...
export const Register: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      addNotification("Google connection failed: " + (err.message || "Unknown error"), "error");
      setLoading(false);
    }
  };
        ...
        <GlassCard className="w-full max-w-sm p-8 text-center space-y-6 animate-in zoom-in duration-300">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed">
              We've sent a confirmation link to <span className="text-gray-900 dark:text-white font-bold">{formData.email}</span>. 
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4 bg-[#f4f7f9] dark:bg-slate-950 transition-colors duration-300">
      <div className="w-full max-w-sm space-y-8 animate-in slide-in-from-right duration-300 pt-10 pb-20">
        <Link to="/login" className="inline-flex items-center gap-2 text-gray-400 dark:text-slate-400 hover:text-gray-800 dark:hover:text-white transition-colors">
          <ArrowLeft size={16} />
          <span className="text-sm">Back to Login</span>
        </Link>
        
        <div className="text-center">
          <h1 className="text-3xl font-black mb-2 text-gray-900 dark:text-white">Create Account</h1>
          <p className="text-gray-500 dark:text-slate-400 text-sm">Join the community of 10k+ gamers</p>
        </div>
        ...
        {errorMsg && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 rounded-xl flex items-start gap-3 animate-in slide-in-from-top-2">
            ...
            <p className="text-xs text-red-700 dark:text-red-300 leading-relaxed font-medium">{errorMsg}</p>
          </div>
        )}
        ...
        <GlassCard className="p-8">
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="relative">
              <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500" size={18} />
              <input 
                type="text" required
                value={formData.name}
            ...
            >
              {loading && formData.email ? <Loader2 className="animate-spin" size={20} /> : 'CREATE ACCOUNT'}
            </button>
          </form>

          <div className="my-6 flex items-center gap-4">
            <div className="h-[1px] flex-1 bg-gray-100 dark:bg-slate-700" />
            <span className="text-[10px] font-bold text-gray-300 dark:text-slate-600 uppercase tracking-widest">Or register with</span>
            <div className="h-[1px] flex-1 bg-gray-100 dark:bg-slate-700" />
          </div>
        ...
        <p className="text-center text-sm text-gray-500 dark:text-slate-500">
          By registering, you agree to our <span className="text-[#ff8c00] dark:text-orange-400 font-bold">Terms of Service</span>
        </p>
      </div>
    </div>
  );
